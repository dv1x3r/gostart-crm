...
export function createProductGrid() {
  return new w2grid({
    ...
    columns: [
      ...
      {
        field: 'name', text: 'Name', size: '300px', render: 'hover', sortable: true,
        info: {
          // showOn: 'mouseenter',
          render: row => {
...
function createProductDetailsForm(tabID, id, attributeGrid) {
  ...
  const detailsForm = new w2form({
    ...
    fields: [
      ...
      { field: 'product_url', html: { label: 'Slug', span: 3, column: 0, attr: 'style="width: 100%; min-width:203px;" readonly' }, type: 'text' },
      ...
      { field: 'created_at', html: { label: 'Created at', span: 4, column: 2, attr: 'readonly' }, type: 'datetime' },
    ...
    _save: async () => {
      ...
      if (attributeGrid.getChanges().length > 0) {
...
export function openProductStatusPopup() {
  ...
  w2popup.open({
    ...
    showMax: true,
    resizable: true,
    body: '<div id="product-status-grid" style="width: 100%; height: 100%;"></div>'
  })
